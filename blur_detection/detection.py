...
def estimate_blur(image: numpy.array, threshold: int = 100):
    """
    Estimate the blur of an image

    Args:
        image (numpy.array): Image to estimate the blur of converted to an cv2.array
        threshold (int, optional): Threshold for the blur to be considered low. Defaults to 100.
...
def pretty_blur_map(blur_map: numpy.array,
                    sigma: int = 5,
                    min_abs: float = 0.5):
